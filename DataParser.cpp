...
int DataParser::getResources(DataResources &resources, DataTuples &tuples)
{
	...
	while (!_inputData.eof())
	{
		...
		else
		{
			...
			case SUBJECTS:
			{
				...
				if (teacher == nullptr)
				{
					cout << "Room " << columns[2] << " not existing - subject " << name
						 << endl;
					return -2;
				}
			...
			case TUPLES:
			{
				...
				if (subject == nullptr)
				{
					cout << "Class " << columns[0] << " not existing - subject "
						 << columns[1] << endl;
					return -2;
				}
